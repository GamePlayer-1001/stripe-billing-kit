...
- **对账兜底**：`reconcile()` 挂 cron 全量核对 Stripe 与本地状态，修复 webhook 丢失导致的权益漂移
...
| 可变价格额度包 | `credit_variable` | 用户自选金额充值 |
| 单日通行证 | `daily` | 日票、单次活动访问 |
| 新人专享试用 | `first_trial` | 只能订阅一次的首单优惠 |

**5 分钟接入示例：**

```ts
import { createCheckoutSession, reportUsage, getCreditBalance } from '@billing-kit/core';
